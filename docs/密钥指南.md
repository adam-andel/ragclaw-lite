...
`.env.example` 描述可配置的环境变量；密钥的完整契约（`ragclaw_config_key`、`config.enc` 格式、备份/轮换流程）以本文档为准——如果你在找「`ragclaw_config_key` 该怎么备份 / 能不能换」之类的问题，答案都在这里。
...
| Per-Skill API Key（`skill_secrets`） | 同上（与 LLM/Embedding key 同文件） | **否** | **仅通过技能配置入口设置**；加密写入 `config.enc`，不暴露给前端 |
...
**关键结论**：API Key（LLM / Embedding）**主要通过 Settings UI 写入 `config.enc` 管理**。`LLM_API_KEY` 可作为可选默认种子在 `.env` 中配置，但 UI 设置的值始终优先覆盖 `.env`。首次启动时，若 `config.enc` 不存在，则以 `.env` 中 `LLM_API_KEY` 的值作为默认（未配置则为空）。
...
- **优先来源**：设置页 UI。系统从 Settings UI 读取并加密存储到 `config.enc`；同时支持从 `.env` 的 `LLM_API_KEY` 作为可选默认种子（UI 设置后会覆盖该默认值）。
...
- **config.enc v1 格式（字节布局）**：
  ...
  加密存储的字段包括：`llm_api_key`、`embedding_api_key`、`skill_secrets`（per-skill API Key 命名空间）。
...
1. **必须单独备份两样东西**：
   ...
   - `ragclaw_data` 卷（含 `config.enc` + sqlite）——加密的 key（含 LLM / Embedding / Per-Skill）与全部配置。
...
- **`.env` 中的 `LLM_API_KEY` 为可选默认种子**：首次启动时，若 `config.enc` 不存在，系统会读取 `.env` 中 `LLM_API_KEY` 作为初始默认值。通过 Settings UI 设置后，该值会加密写入 `config.enc` 并覆盖 `.env` 默认值。

---

## 9. 安全边界（不要做的事 / Do NOT）

- 不要在 `.env` 中提交真实的 `LLM_API_KEY`（建议通过 Settings UI 加密存储；`.env` 中的值仅作为未配置 UI 时的可选默认种子）。
- 不要把 `secrets/` 提交进 git。
